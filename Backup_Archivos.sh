...
function Archivar2 {
    ...
    ls -ad $carpeta/*/ > directorios.txt
    
    for line in $(cat directorios.txt); do
        if [ -f $line*.$extension ]; then
            echo "Guardando contenido de: $line"
                tar -rvf "$dir/$name.tar" $line*.$extension
        else
            echo -e "\e[1;31mCarpeta $line sin archivos .$extension\e[0m"
        fi
    done    
    rm directorios.txt
    echo -e "\e[1;42m Respaldo Efectuado con exito en $dir/$name.tar \e[0m"
